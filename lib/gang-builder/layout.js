  ...
  if (tryRotated && placedCount < quantity) {
    ...
    for (let row = 0; row < rotatedRows && placedCount < quantity; row++) {
      for (let col = 0; col < rotatedCols && placedCount < quantity; col++) {
        // Calculate the bounding box position (top-left of the rotated bounding box, including deadspace)
        // This should match the pattern used for non-rotated instances
        const rotatedBoxXIn = col * rotatedCellW + paddingIn / 2 + deadspaceIn;
        const rotatedBoxYIn = row * rotatedCellH + paddingIn / 2 + deadspaceIn;
        
        // The bounding box center (accounting for deadspace being included in the box position)
        const boxCenterX = rotatedBoxXIn - deadspaceIn + rotatedWidth / 2;
        const boxCenterY = rotatedBoxYIn - deadspaceIn + rotatedHeight / 2;
        ...
        // Check if this position overlaps with existing positions (use bounding box without deadspace offset for overlap check)
        if (!overlapsWithExisting(
          rotatedBoxXIn - deadspaceIn,
          rotatedBoxYIn - deadspaceIn,
          rotatedWidth,
          rotatedHeight,
          occupiedAreas
        )) {
          ...
          occupiedAreas.push({
            xIn: rotatedBoxXIn - deadspaceIn,
            yIn: rotatedBoxYIn - deadspaceIn,
