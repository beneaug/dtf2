...
export function autoPackDesign({
  ...
  // Phase 2: Try rotated orientation if requested
  ...
  if (tryRotated && (placedCount < quantity || isMaxCalculation)) {
    ...
    for (let row = 0; row < rotatedRows; row++) {
      for (let col = 0; col < rotatedCols; col++) {
        ...
        // For rotated graphics, calculate the graphic position
        // The graphic position (xIn, yIn) is the top-left in the ORIGINAL orientation
        // After 90-degree rotation, the graphic occupies a different space
        
        // Calculate where the graphic should be placed
        // We'll place it such that when rotated, it fits in the cell
        const cellX = col * rotatedCellW + paddingIn / 2;
        const cellY = row * rotatedCellH + paddingIn / 2;
        
        // For a 90-degree rotated graphic:
        // - The bounding box is: rotatedEffectiveWidth x rotatedEffectiveHeight
        // - The graphic's original dimensions are: designWidthIn x designHeightIn
        // - After rotation, the graphic's footprint is: designHeightIn x designWidthIn
        // - The graphic position should be such that when rotated around its center, it fits in the bounding box
        
        // Calculate graphic center (which stays the same after rotation)
        const boxCenterX = cellX + rotatedEffectiveWidth / 2;
        const boxCenterY = cellY + rotatedEffectiveHeight / 2;
        
        // Graphic position (top-left in original orientation)
        // After 90-degree rotation, the graphic's top-left in original orientation
        // becomes the top-right of the rotated graphic
        // So: graphicX = boxCenterX - designHeightIn / 2
        //     graphicY = boxCenterY - designWidthIn / 2
