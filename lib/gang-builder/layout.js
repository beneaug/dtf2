  ...
  if (tryRotated && placedCount < quantity) {
    ...
    for (let row = 0; row < rotatedRows && placedCount < quantity; row++) {
      for (let col = 0; col < rotatedCols && placedCount < quantity; col++) {
        // For rotated instances, the position is the top-left of the original (unrotated) graphic
        // When rotated 90 degrees, the graphic's top-left becomes the top-right of the bounding box
        // So we need to adjust: xIn should account for the fact that after rotation, 
        // the graphic's left edge is actually at xIn - rotatedHeight (since height becomes width)
        // But we want xIn to be the top-left of the original graphic orientation
        const rotatedXIn = col * rotatedCellW + paddingIn / 2 + deadspaceIn;
        const rotatedYIn = row * rotatedCellH + paddingIn / 2 + deadspaceIn;
        
        // The position stored should be the top-left of the original graphic (before rotation)
        // When rotated 90 degrees clockwise, the graphic's top-left is at:
        // x = rotatedXIn - (rotatedHeight - designHeightIn) = rotatedXIn - (effectiveHeight - designHeightIn)
        // But actually, we want the position to be where the graphic's top-left corner is
        // After rotation, the graphic's top-left in original orientation is at:
        // x = rotatedXIn - (rotatedHeight - designHeightIn) / 2
        // Actually, let's think differently: the position should be the top-left of the graphic
        // When we rotate 90 degrees, the graphic's original top-left moves
        // Let's use the center-based approach: store the center, then calculate top-left
        // Or simpler: store the position as if it's not rotated, then adjust in rendering
        
        // Actually, the simplest: store the position where the graphic's top-left would be
        // For a 90-degree rotation, if the bounding box top-left is at (rotatedXIn, rotatedYIn),
        // and the bounding box is rotatedWidth x rotatedHeight,
        // then the graphic's original top-left (before rotation) is at:
        // x = rotatedXIn + rotatedHeight/2 - designWidthIn/2
        // y = rotatedYIn + rotatedWidth/2 - designHeightIn/2
        
        // Wait, let's reconsider. The instance.xIn should be the top-left of the graphic itself.
        // When rotated 90 degrees clockwise:
        // - The graphic's original top-left moves to where the right edge of the rotated bounding box is
        // - More precisely: if the rotated bounding box center is at (cx, cy),
        //   and the graphic's original size is w x h,
        //   then the graphic's original top-left is at (cx - h/2, cy - w/2)
        
        const centerX = rotatedXIn + rotatedWidth / 2;
        const centerY = rotatedYIn + rotatedHeight / 2;
        const xIn = centerX - designHeightIn / 2; // After rotation, height becomes the x dimension
        const yIn = centerY - designWidthIn / 2;   // After rotation, width becomes the y dimension
        
        // Check if this position overlaps with existing positions
        if (!overlapsWithExisting(
          rotatedXIn - deadspaceIn,
          rotatedYIn - deadspaceIn,
          rotatedWidth,
          rotatedHeight,
          occupiedAreas
        )) {
          ...
          occupiedAreas.push({
            xIn: rotatedXIn - deadspaceIn,
            yIn: rotatedYIn - deadspaceIn,
