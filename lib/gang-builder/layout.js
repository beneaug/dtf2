...
export function autoPackDesign({
  ...
  // Calculate theoretical grid size, but we'll verify each position actually fits
  const cols = Math.floor(sheetWidthIn / cellW);
  const rows = Math.floor(sheetHeightIn / cellH);
  ...
  let placedCount = 0;
  let maxFitCount = 0; // Count of positions that actually fit (for max calculation)
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      // Stop placing if we've reached the requested quantity (unless calculating max)
      if (!isMaxCalculation && placedCount >= quantity) break;
      
      // Calculate bounding box position (top-left of bounding box)
      ...
      // Use strict comparison to ensure no part goes outside
      if (boxXIn < 0 || boxYIn < 0 || 
          boxXIn + effectiveWidth > sheetWidthIn || 
          boxYIn + effectiveHeight > sheetHeightIn) {
        continue; // Skip positions that go off the sheet
      }
      
      // Verify using isWithinBounds for consistency
      if (!isWithinBounds(boxXIn, boxYIn, effectiveWidth, effectiveHeight, sheetWidthIn, sheetHeightIn)) {
        continue; // Skip positions that go off the sheet
      }
      
      // Second check: Does this position overlap with existing positions?
      ...
      // Position is valid - count it for max calculation
      maxFitCount++;
  ...
  // Use actual count of positions that fit, not theoretical grid size
  ...
  if (tryRotated && (placedCount < quantity || isMaxCalculation)) {
    ...
    // Calculate theoretical grid size for rotated orientation
    const rotatedCols = Math.floor(sheetWidthIn / rotatedCellW);
    const rotatedRows = Math.floor(sheetHeightIn / rotatedCellH);
    ...
    for (let row = 0; row < rotatedRows; row++) {
      for (let col = 0; col < rotatedCols; col++) {
        ...
        // STRICT bounds check: entire bounding box must be within sheet
        if (rotatedBoxXIn < 0 || rotatedBoxYIn < 0 || 
            rotatedBoxXIn + rotatedWidth > sheetWidthIn || 
            rotatedBoxYIn + rotatedHeight > sheetHeightIn) {
          continue; // Skip positions that go off the sheet
        }
        ...
        // Position is valid - calculate graphic position
        ...
        // STRICT verification: graphic bounding box must be entirely within sheet
        const graphicBoxX = xIn - deadspaceIn;
        const graphicBoxY = yIn - deadspaceIn;
        const graphicBoxWidth = designHeightIn + (deadspaceIn * 2); // Swapped for rotated
        const graphicBoxHeight = designWidthIn + (deadspaceIn * 2); // Swapped for rotated
        
        if (graphicBoxX < 0 || graphicBoxY < 0 || 
            graphicBoxX + graphicBoxWidth > sheetWidthIn || 
            graphicBoxY + graphicBoxHeight > sheetHeightIn) {
          continue; // Skip if graphic position is out of bounds
        }
        
        // Verify using isWithinBounds for consistency
        if (!isWithinBounds(graphicBoxX, graphicBoxY, graphicBoxWidth, graphicBoxHeight, sheetWidthIn, sheetHeightIn)) {
          continue; // Skip if graphic position is out of bounds
        }
