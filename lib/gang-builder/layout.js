...
function packWithStrategy(params, forcePrimaryRotation) {
  ...
  if (placedCount < quantity || isMaxCalculation) {
    // Use a step size that is fine enough to find gaps but not overkill
    // 1/8th inch (0.125) is usually sufficient for placement
    const stepSize = Math.max(0.0625, paddingIn); 
    
    // Safety break only - make it large enough to cover the whole sheet
    // For a 22x120 sheet with 0.0625 step: (22/0.0625) * (120/0.0625) = ~675,000 points
    ...
    // Scan grid
    for (let y = 0; y <= sheetHeightIn - Math.min(h0, h90); y += stepSize) {
      for (let x = 0; x <= sheetWidthIn - Math.min(w0, w90); x += stepSize) {
        attempts++;
        ...
        // Optimization: if we placed something, advance x?
        // Advancing might miss tight fits, but we can jump at least a small amount
        // to avoid checking immediately overlapping pixels.
      }
      if (attempts > maxAttempts) break;
      if (!isMaxCalculation && placedCount >= quantity) break;
    }
