  ...
  if (tryRotated && placedCount < quantity) {
    ...
    for (let row = 0; row < rotatedRows && placedCount < quantity; row++) {
      for (let col = 0; col < rotatedCols && placedCount < quantity; col++) {
        ...
        // The bounding box position doesn't include deadspace in the calculation
        // Deadspace is added around the graphic, so the bounding box top-left is:
        ...
        // - The graphic's original top-left (before rotation) is at: (centerX - height/2, centerY - width/2)
        // - This is because after rotation, the graphic's height becomes the x-dimension
        // - But we also need to account for deadspace: the graphic is inside the bounding box
        // - So: graphic top-left = bounding box center - (graphic dimensions / 2) - deadspace
        // - Actually simpler: graphic top-left = bounding box top-left + deadspace
        // - But wait, that's not right either. Let me think...
        // - Bounding box top-left = graphic top-left - deadspace
        // - So: graphic top-left = bounding box top-left + deadspace
        // - But the bounding box center is at: boxTopLeft + boxWidth/2
        // - And boxWidth = graphicHeight + 2*deadspace
        // - So: graphic top-left = boxCenter - graphicHeight/2 - deadspace
        // - Which is: graphic top-left = boxCenter - (graphicHeight/2 + deadspace)
        // - But actually: graphic top-left = boxCenter - graphicHeight/2 (because deadspace is already in the box)
        // - No wait, the bounding box includes deadspace, so:
        //   boxCenter = boxTopLeft + boxWidth/2 = (graphicTopLeft - deadspace) + (graphicHeight + 2*deadspace)/2
        //   boxCenter = graphicTopLeft - deadspace + graphicHeight/2 + deadspace
        //   boxCenter = graphicTopLeft + graphicHeight/2
        //   So: graphicTopLeft = boxCenter - graphicHeight/2
        //   But for rotated, graphicHeight becomes the x-dimension, so:
