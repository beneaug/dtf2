...
/**
 * Check if a bounding box is within sheet bounds
 * @param {number} xIn - X position in inches (top-left of bounding box)
 * @param {number} yIn - Y position in inches (top-left of bounding box)
 ...
 */
function isWithinBounds(xIn, yIn, widthIn, heightIn, sheetWidthIn, sheetHeightIn) {
  return (
    xIn >= 0 &&
    yIn >= 0 &&
...
export function autoPackDesign({
  ...
  // Limit iterations to prevent going off sheet - use actual calculated rows/cols
  const maxIterations = rows * cols;
  
  for (let i = 0; i < maxIterations && placedCount < quantity; i++) {
  ...
  if (tryRotated && (placedCount < quantity || isMaxCalculation)) {
    ...
    for (let row = 0; row < rotatedRows; row++) {
      for (let col = 0; col < rotatedCols; col++) {
        ...
        // Safety check: ensure row/col don't exceed calculated limits
        if (row >= rotatedRows || col >= rotatedCols) break;
        ...
        // Position is valid - calculate graphic position and add it
        ...
        // Verify graphic position is also within bounds (double-check)
