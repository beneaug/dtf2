...
export function autoPackDesign({
  ...
  if (tryRotated && (placedCount < quantity || isMaxCalculation)) {
    ...
    if (placedCount < quantity || isMaxCalculation) {
      ...
      // Use smaller steps for better gap detection
      const stepSize = Math.max(0.025, Math.min(paddingIn * 0.25, 0.0625)); // Between 0.025 and 0.0625 inches for finer scanning
      const maxAttempts = 5000; // Increase attempts for better coverage
      let attempts = 0;
