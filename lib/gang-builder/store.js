 ...
 * @property {string|null} selectedInstanceId - Currently selected instance ID (primary)
 * @property {string[]} selectedInstanceIds - All selected instance IDs
...
export function removeDesignFile(id) {
  ...
  state.selectedInstanceIds = state.selectedInstanceIds.filter(instId => {
    return state.instances.some(i => i.id === instId);
  });
 ...
 * @param {Array<{id: string, changes: Partial<PlacedInstance>}>} updates
 */
export function updateInstances(updates) {
  let changed = false;
...
export function setSelectedInstance(id) {
  ...
  state.selectedInstanceIds = id ? [id] : [];
  notifyListeners();
}

/**
 * Set selected instances (multiple selection)
 * @param {string[]} ids
 */
export function setInstanceSelection(ids) {
  state.selectedInstanceIds = [...ids];
  state.selectedInstanceId = ids.length > 0 ? ids[ids.length - 1] : null; // Last selected is primary
  notifyListeners();
}
 ...
 * @param {string} id
 * @param {boolean} multiSelect - If true, toggle; if false, select only this one
 */
export function toggleInstanceSelection(id, multiSelect = true) {
  if (!multiSelect) {
    setSelectedInstance(id);
    return;
  }

  const index = state.selectedInstanceIds.indexOf(id);
  if (index > -1) {
    // Remove
    state.selectedInstanceIds.splice(index, 1);
    if (state.selectedInstanceId === id) {
      state.selectedInstanceId = state.selectedInstanceIds.length > 0 
...
export function removeInstances(ids) {
  ...
  state.selectedInstanceIds = state.selectedInstanceIds.filter(id => !ids.includes(id));
  if (state.selectedInstanceId && ids.includes(state.selectedInstanceId)) {
    state.selectedInstanceId = state.selectedInstanceIds.length > 0 ? state.selectedInstanceIds[state.selectedInstanceIds.length - 1] : null;
  }
  notifyListeners();
}
