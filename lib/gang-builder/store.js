 ...
 * @property {string|null} selectedInstanceId - Currently selected instance ID (single selection)
 * @property {Set<string>} selectedInstanceIds - Set of selected instance IDs (multi-selection)
...
const defaultState = {
  ...
  selectedInstanceIds: new Set(), // Multi-selection support
...
function notifyListeners() {
  listeners.forEach((listener) => {
    try {
      // Create a copy of state with Set properly cloned
      const stateCopy = { 
        ...state,
        selectedInstanceIds: new Set(state.selectedInstanceIds),
      };
      listener(stateCopy);
    } catch (error) {
...
export function getState() {
  ...
  return { 
    ...state,
    selectedInstanceIds: new Set(state.selectedInstanceIds), // Create new Set from existing
  };
}
...
/**
 * Set selected instance (single selection)
 * @param {string|null} id
 */
export function setSelectedInstance(id) {
  state.selectedInstanceId = id;
  if (id) {
    state.selectedInstanceIds = new Set([id]);
  } else {
...
/**
 * Toggle instance selection (for multi-select with Ctrl/Cmd)
 * @param {string} id
 */
export function toggleInstanceSelection(id) {
  if (state.selectedInstanceIds.has(id)) {
    state.selectedInstanceIds.delete(id);
    if (state.selectedInstanceIds.size === 0) {
      state.selectedInstanceId = null;
    } else if (state.selectedInstanceId === id) {
      // If we deselected the primary selection, pick the first remaining
      state.selectedInstanceId = Array.from(state.selectedInstanceIds)[0];
    }
  } else {
    state.selectedInstanceIds.add(id);
    state.selectedInstanceId = id; // Set as primary selection
  }
  notifyListeners();
}
 ...
 * @param {string} id
 */
export function addInstanceToSelection(id) {
  state.selectedInstanceIds.add(id);
  state.selectedInstanceId = id;
  notifyListeners();
}
 ...
 * @param {string[]} ids
 */
export function setSelectedInstances(ids) {
  state.selectedInstanceIds = new Set(ids);
  state.selectedInstanceId = ids.length > 0 ? ids[0] : null;
  notifyListeners();
}
 ...
 */
export function rotateSelectedInstances(degrees = 90) {
  ...
  selectedIds.forEach(id => {
    const instance = state.instances.find(i => i.id === id);
    if (instance) {
      const newRotation = ((instance.rotationDeg || 0) + degrees) % 360;
      updateInstance(id, { rotationDeg: newRotation });
    }
  });
}
 ...
 * @param {string[]} ids
 */
export function removeInstances(ids) {
  state.instances = state.instances.filter((i) => !ids.includes(i.id));
  if (state.selectedInstanceId && ids.includes(state.selectedInstanceId)) {
    state.selectedInstanceId = null;
  }
  ids.forEach(id => state.selectedInstanceIds.delete(id));
  notifyListeners();
}
...
export function resetAll() {
  state = { 
    ...defaultState,
    selectedInstanceIds: new Set(), // Ensure Set is properly initialized
  };
  notifyListeners();
}
