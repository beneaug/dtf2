...
export function addInstancesForDesign(designId, quantity, autoPack = false) {
  ...
  for (const pos of positions) {
    // Final validation: ensure instance is within bounds
    ...
    if (boxX < 0 || boxY < 0 || 
        boxX + boxWidth > sheetSize.widthIn || 
        boxY + boxHeight > sheetSize.heightIn) {
      console.warn(`Skipping instance at (${pos.xIn}, ${pos.yIn}) - out of bounds. Box: (${boxX.toFixed(3)}, ${boxY.toFixed(3)}) size: ${boxWidth.toFixed(3)}x${boxHeight.toFixed(3)}, sheet: ${sheetSize.widthIn}x${sheetSize.heightIn}`);
      continue; // Skip invalid positions
    }
    
    // Additional verification: ensure graphic position coordinates are valid
    // (The bounding box check above already ensures the full footprint fits)
