...
export function addInstancesForDesign(designId, quantity, autoPack = false) {
  ...
  const existingOccupiedAreas = state.instances
    ...
    .map((inst) => {
      ...
      // For rotated instances, swap width/height for the bounding box
      const boxWidth = (isRotated ? inst.heightIn : inst.widthIn) + (deadspaceIn * 2);
      const boxHeight = (isRotated ? inst.widthIn : inst.heightIn) + (deadspaceIn * 2);
      
      // Calculate bounding box top-left
      const boxX = inst.xIn - deadspaceIn;
      const boxY = inst.yIn - deadspaceIn;
  ...
  if (autoPack) {
    ...
    // Place instances in a simple grid if not auto-packing, checking for overlaps
    ...
    for (let row = 0; row < 100 && placed < quantity && attempts < maxAttempts; row++) {
      for (let col = 0; col < cols && placed < quantity && attempts < maxAttempts; col++) {
        attempts++;
        const xIn = col * spacing;
        const yIn = row * spacing;
        
        // Check bounds
        if (xIn + effectiveWidth > sheetSize.widthIn || yIn + effectiveHeight > sheetSize.heightIn) {
          continue;
        }
        
        // Check overlap with existing instances
        const boxX = xIn - deadspaceIn;
        const boxY = yIn - deadspaceIn;
        let overlaps = false;
        ...
        if (!overlaps) {
          positions.push({
            xIn: xIn + deadspaceIn, // Graphic position includes deadspace offset
            yIn: yIn + deadspaceIn,
            rotated: 0,
          });
          placed++;
  ...
  for (const pos of positions) {
    // Validate position is within bounds before creating instance
    const isRotated = pos.rotated === 90;
    const boxWidth = (isRotated ? designHeightIn : designWidthIn) + (deadspaceIn * 2);
    const boxHeight = (isRotated ? designWidthIn : designHeightIn) + (deadspaceIn * 2);
    const boxX = pos.xIn - deadspaceIn;
    const boxY = pos.yIn - deadspaceIn;
    
    // Final bounds check - skip if out of bounds
    if (!isWithinBounds(boxX, boxY, boxWidth, boxHeight, sheetSize.widthIn, sheetSize.heightIn)) {
      console.warn(`Skipping instance at (${pos.xIn}, ${pos.yIn}) - out of bounds`);
      continue;
    }
