...
export function getSheetUsage(state) {
  ...
  // Include 4mm (0.157 inches) deadspace on all sides for each instance
  const deadspaceIn = 0.157;
  let usedAreaIn = 0;
  for (const instance of instances) {
    // Add deadspace to both width and height (deadspace on all sides)
    const widthWithDeadspace = (instance.widthIn || 0) + (deadspaceIn * 2);
    const heightWithDeadspace = (instance.heightIn || 0) + (deadspaceIn * 2);
    const area = widthWithDeadspace * heightWithDeadspace;
    usedAreaIn += area;
  }
